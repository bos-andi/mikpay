...
/*
 * Print Report Page
 * Generates a printable yearly/monthly report with F4 paper size
 */
session_start();
error_reporting(0);
...
// Get yearly data
$yearlyData = getYearlySummary($year);
$totalYearly = array_reduce($yearlyData, function($sum, $m) { return $sum + $m['total_amount']; }, 0);
$totalTransactions = array_reduce($yearlyData, function($sum, $m) { return $sum + $m['total_transactions']; }, 0);
...
<head>
    ...
    <title>Laporan Tahunan <?= $year ?> - <?= htmlspecialchars($businessName) ?></title>
    <style>
        ...
        * {
        ...
        .data-table th:nth-child(1) { width: 10%; text-align: center; }
        .data-table th:nth-child(2) { width: 40%; }
        .data-table th:nth-child(3) { width: 20%; text-align: center; }
        .data-table th:nth-child(4) { width: 30%; text-align: right; }
        ...
        .data-table td:nth-child(4) { text-align: right; font-weight: 600; }
    ...
    <div class="page-container">
        ...
        <div class="summary-section">
            <p>
                ...
                <?= htmlspecialchars($businessName) ?> selama periode tahun <?= $year ?>.
            </p>
            
            <div class="summary-box">
                <div class="summary-item">
                    <div class="label">Total Pendapatan</div>
                    <div class="value">Rp <?= number_format($totalYearly, 0, ',', '.') ?></div>
                </div>
                <div class="summary-item">
        ...
        <table class="data-table">
            ...
            <tbody>
                <?php foreach ($yearlyData as $month => $data): ?>
                <tr class="<?= $data['total_transactions'] == 0 ? 'no-data' : '' ?>">
                    <td><?= $month ?></td>
                    <td><?= $monthNames[$month] ?> <?= $year ?></td>
                    <td><?= $data['total_transactions'] > 0 ? $data['total_transactions'] : '-' ?></td>
                    <td><?= $data['total_transactions'] > 0 ? number_format($data['total_amount'], 0, ',', '.') : '-' ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
            <tfoot>
                <tr>
                    <td colspan="2" style="text-align: center;"><strong>TOTAL TAHUN <?= $year ?></strong></td>
                    <td style="text-align: center;"><strong><?= $totalTransactions ?></strong></td>
                    <td><strong><?= number_format($totalYearly, 0, ',', '.') ?></strong></td>
                </tr>
            </tfoot>
        </table>
